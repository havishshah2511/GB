...
Reported bug: two buyers wanting the same rice in the same city landed in
separate groups (RICE-VAD-001 and RICE-VAD-002) because one said "within 7
days" and the other "within 15 days". Their windows were computed as
24-31 Aug and 1-8 Sep -- adjacent, non-overlapping -- so the matcher rejected
the group, the quantities never combined, the price never dropped and nobody
was notified.
...
RICE = {
    ...
    "can_wait": "yes", "package_size": "25 kg",
}


def rice_buyer(chat, session, opening, when, name, mobile):
    return answer_all(chat, session, opening, {
        **RICE, "desired_purchase_date": when, "name": name, "mobile": mobile,
    })
...
def test_different_deadlines_still_share_one_group(chat):
    rice_buyer(chat, "w1", "I need 33 kg basmati rice", "Within 7 days",
               "Havish", "9000000001")
    rice_buyer(chat, "w2", "I need 100 kg basmati rice", "Within 15 days",
               "Harsh", "9000000002")
    ...
    assert all_groups[0]["strong_intent_qty"] == 133


def test_pooling_drops_the_price_and_notifies_the_earlier_buyer(chat):
    rice_buyer(chat, "w3", "I need 33 kg basmati rice", "Within 7 days",
               "Havish", "9000000003")
    before = groups.list_groups()[0]
    assert before["current_price"] == 95

    rice_buyer(chat, "w4", "I need 100 kg basmati rice", "Within 15 days",
               "Harsh", "9000000004")
    after = groups.get(before["id"])
    assert after["current_price"] == 90, "combined quantity did not reprice the group"
...
def test_any_pair_of_deadlines_pools(chat, first, second):
    rice_buyer(chat, "p1", "I need 40 kg basmati rice", first, "A", "9000001001")
    rice_buyer(chat, "p2", "I need 40 kg basmati rice", second, "B", "9000001002")
...
def test_a_deadline_answer_makes_the_buyer_available_from_today(chat):
    rice_buyer(chat, "w5", "I need 50 kg basmati rice", "Within 15 days",
               "Chetan", "9000000005")
...
def test_an_explicit_date_is_respected_as_the_earliest(chat):
    ...
    rice_buyer(chat, "w6", "I need 50 kg basmati rice", target, "Deepa", "9000000006")
    intent = intents.list_intents()[0]
...
def test_group_window_spans_from_the_earliest_member_date(chat):
    rice_buyer(chat, "w7", "I need 40 kg basmati rice", "Within 7 days",
               "Esha", "9000000007")
    rice_buyer(chat, "w8", "I need 40 kg basmati rice", "Within 30 days",
               "Farhan", "9000000008")
...
def test_a_far_future_buyer_does_not_join_an_urgent_group(chat):
    ...
    rice_buyer(chat, "w9", "I need 40 kg basmati rice", "Within 3 days",
               "Gita", "9000000009")
    far = str(today() + timedelta(days=120))
    rice_buyer(chat, "w10", "I need 40 kg basmati rice", far, "Hemant", "9000000010")
...
def test_a_brand_locked_buyer_still_gets_their_own_group(chat):
    answer_all(chat, "b1", "I need 40 kg basmati rice", {
        **RICE, "desired_purchase_date": "Within 7 days",
        "name": "Ira", "mobile": "9000002001",
    })
    answer_all(chat, "b2", "I need 40 kg basmati rice", {
        **RICE, "brand_flexible": "no", "desired_purchase_date": "Within 7 days",
